...
function get_sv_id_by_location(lng, lat) {
    ...
    base_lat = lat;
    base_lng = lng;
    loadJSON('../routes/godovic.json', function(path) {
    ...
    for (var n = 0; n < all_pano_ids.length; ++n) {
        ...
        var depth_promise = new Promise(function(resolve, reject) {
            ...
            depth_loader.onDepthLoad = function() {
                        ...
                        console.log("Loaded depth data", cur_depth_elem + 1, "of", all_pano_ids.length + " ["+self.n+"] elevation: " + self.elevation + "lat/lng: " + self.data.Location.lat + "," + self.data.Location.lng );
                        ...
                        var base_offset = self.n * depthmap_width * depthmap_height / (point_step * point_step);
                        ...
                        for (var y = 0, num = 0; y < depthmap_height; y += point_step) {
                            ...
                            for (var x = 0; x < depthmap_width; x += point_step) {
                                ...
                                var depth = parseFloat(self.depthMap.depthMap[y * depthmap_width + depthmap_width - x]);
